...
import { 
  ...
  Activity, Info, ChevronRight, Package, DollarSign, Target, Box
} from 'lucide-react';
...
export default function HomeSection({ onNavigate }: any) {
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      {/* --- 2. DYNAMIC KPI STRIP (DRILL-DOWN) --- */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <KPIButton 
          title="Stockout Rate" val={stats?.stockout_rate || "2.1%"} change="-0.5%" 
          icon={<AlertTriangle size={16}/>} color="rose"
          onClick={() => onNavigate('optimizer')} 
        />
        <KPIButton 
          title="Holding Cost" val={stats?.holding_cost || "$14.2k"} change="+3%" 
          icon={<DollarSign size={16}/>} color="indigo"
          onClick={() => onNavigate('optimization')} 
        />
        <KPIButton 
          title="Inv. Turnover" val={stats?.turnover || "8.4x"} change="+12%" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 px-2">
        ...
        <MetricDef label="Holding Cost" desc="Total capital tied in warehousing/unsold units." />
             ...
             <table className="w-full text-left">
                ...
                <tbody className="text-xs font-bold text-slate-700">
                   {stats?.risk_skus ? stats.risk_skus.map((risk: any, i: number) => (
                      ...
                      <>
                        <RiskRow id="NOV-001" issue="High CV (0.42)" impact="Critical" color="text-rose-600" />
                        <RiskRow id="NOV-042" issue="Lead Time Delay" impact="High" color="text-amber-600" />
                        <RiskRow id="NOV-089" issue="Overstocked" impact="Capital" color="text-indigo-600" />
                      </>
                   )}
                </tbody>
             </table>
...
function ActionCard({ title, impact, desc, btnLabel, onAction }: any) {
  return (
    <div className="p-8 rounded-[3rem] border-2 bg-indigo-50 border-indigo-100 shadow-xl flex flex-col md:flex-row items-center justify-between gap-6 hover:scale-[1.01] transition-transform">
      ...
      <button onClick={onAction} className="bg-slate-900 text-white px-6 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 transition-all flex items-center gap-2 shrink-0 active:scale-95 shadow-lg shadow-slate-900/10">
