...
import { 
  UploadCloud, FileSpreadsheet, Database, CheckCircle2, 
  BarChart3, PieChart, RefreshCcw, Download, AlertTriangle, FileText 
} from 'lucide-react';
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        console.log("📊 Data Lab Received:", data);
        setReport(data);
      } catch (err: any) {
          ...
          <div className={`border-4 border-dashed rounded-[3rem] p-20 text-center transition-all ${isanalyzing ? 'border-indigo-400 bg-indigo-50/30' : 'border-slate-200 hover:border-indigo-300 bg-white'}`}>
            {isanalyzing ? (
              <div className="flex flex-col items-center">
                ...
                <p className="text-xl font-black text-indigo-600 animate-pulse uppercase tracking-tighter">Running Analytics Engine...</p>
              </div>
            ) : (
              <>
        ...
        <div className="grid grid-cols-12 gap-6 animate-in slide-in-from-bottom-10 duration-700">
          ...
          <div className="col-span-12 bg-emerald-50 border border-emerald-100 p-8 rounded-[2.5rem] flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-5">
              ...
              <div>
                <h4 className="font-black text-emerald-900 text-xl tracking-tight leading-tight uppercase italic">Batch Analysis Complete</h4>
                <p className="text-sm text-emerald-700 font-medium">Processed {report.filename} successfully.</p>
              </div>
            </div>
          ...
          <div className="col-span-12 md:col-span-3 bg-rose-50 border border-rose-100 p-8 rounded-[2rem] shadow-sm">
             ...
             <h3 className="text-3xl font-black text-rose-900 tracking-tighter">{report.at_risk} <span className="text-xs opacity-50">SKUs</span></h3>
          </div>

          {/* Charts Row */}
          <div className="col-span-12 lg:col-span-5 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm h-[400px]">
            <h4 className="text-xs font-black uppercase text-slate-400 mb-8 tracking-widest italic">Risk Segmentation</h4>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={report.risk_chart}>
                  ...
                  <Bar dataKey="count" radius={[10, 10, 0, 0]} barSize={40}>
                    {report.risk_chart?.map((entry: any, index: number) => (
          ...
          <div className="col-span-12 lg:col-span-7 bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm h-[400px]">
            <h4 className="text-xs font-black uppercase text-slate-400 mb-8 tracking-widest italic">Inventory Profile (ROP Scatter)</h4>
            <div className="h-[280px]">
              <ResponsiveContainer width="100%" height="100%">
                <ScatterChart margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis type="number" dataKey="demand" name="Demand" tick={{fontSize: 10}} label={{ value: 'Demand Units', position: 'insideBottom', offset: -5, fontSize: 10, fill: '#94a3b8' }} />
                  <YAxis type="number" dataKey="lead_time" name="LT" tick={{fontSize: 10}} label={{ value: 'Lead Time', angle: -90, position: 'insideLeft', fontSize: 10, fill: '#94a3b8' }} />
          ...
          <div className="col-span-12 bg-slate-900 text-white p-10 rounded-[3rem] flex flex-col md:flex-row items-center justify-between shadow-2xl mt-4">
            <div className="flex items-center gap-6">
               ...
               <div>
                 ...
                 <p className="text-slate-400 font-medium text-sm">Deploy thresholds for all {report.skus} items into production.</p>
               </div>
            </div>
            <button className="mt-8 md:mt-0 bg-indigo-600 hover:bg-indigo-700 px-10 py-5 rounded-2xl font-black transition-all uppercase text-xs tracking-[0.2em] shadow-xl shadow-indigo-600/30">
